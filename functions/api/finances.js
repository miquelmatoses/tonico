...
const CAMPS = ['caixa', 'caixa_data', 'despesa_planter', 'despesa_estadi', 'ingres_setmanal'];

export async function onRequestGet({ env, data }) {
  const f = await env.DB.prepare(
    'SELECT caixa, caixa_data, despesa_planter, despesa_estadi, ingres_setmanal FROM finances WHERE usuari_id=?'
...
export async function onRequestPost({ request, env, data }) {
  ...
  await env.DB.prepare(
    `INSERT INTO finances (usuari_id, caixa, caixa_data, despesa_planter, despesa_estadi, ingres_setmanal)
     VALUES (?, ?, ?, ?, ?, ?)
  ...
  ).bind(data.usuari.id, enter(v.caixa), v.caixa_data || null, enter(v.despesa_planter), enter(v.despesa_estadi), enter(v.ingres_setmanal)).run();
  return json({ ok: true });
}
