...
// setmanes_restants: es DERIVA de la data de fi i del dia de veres. Abans era un COMPTE
// declarat que ningú decrementava, així que es quedava congelat i el venciment no arribava mai.
// Sense data no es suposa res: null, i qui ho consumix ho ha de dir.
export function setmanesRestants(dataFi, hui) {
  if (!dataFi || !hui) return null;
  const dies = (new Date(dataFi) - new Date(hui)) / 86400000;
  if (!Number.isFinite(dies)) return null;
  return Math.ceil(dies / 7);
}
