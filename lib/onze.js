...
// compatible(rol, lloc): un porter només a porteria, i a porteria només porters. La resta
// de rols poden ocupar qualsevol lloc de camp (el valor ja els ordena).
export function compatible(jugador, lloc, habilitatPorter = 'porteria') {
  const esLlocPorter = lloc.habilitat === habilitatPorter;
  const esPorter = jugador.categoria === 'porter';
  return esLlocPorter === esPorter || (!esLlocPorter && !esPorter);
}
...
export function alineaOnzes(retinguts, llocs, partits, opts = {}) {
  const { pes_entrenament = 1000, habilitat_porter = 'porteria',
  ...
  const maxDe = (j, candidat) => {
    ...
    return Math.min(...pcts.map((pct) => maxPartits(j.categoria, pct === 0 ? undefined : pct)));
  };
  const posa = (j, l) => {
    (assignats[j.jugador_id] ||= []).push({ partit: l.partit, lloc: l.lloc, pct: pctDe(l) });
    onze[l.partit].push({ ...l, jugador: j });
  };
  ...
  for (const l of totsElsLlocs) {
    ...
    const disponible = (j) => !vetat.has(j.jugador_id)
      ...
      && compatible(j, l, habilitat_porter);
    ...
    else onze[l.partit].push({ ...l, jugador: null });               // buit: es juga amb un menys
  }
