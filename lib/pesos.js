...
// ── EL DESCOMPTE DE SOU PER EDAT (wiki «Wages») ──────────────────────────────────────────
// Des dels 29 el sou baixa un 10% per any fins al 37. O siga que el MATEIX pressupost compra un
// jugador molt millor si el busques major: als 32 (−40%) el central passa de CR4/DF8 a CR8/DF9.
// Per davall de 28 no hi ha descompte; passats els 37 es queda com als 37.
export function descompteEdat(edat, taula) {
  if (!taula || edat == null) return 1;
  const claus = Object.keys(taula).map(Number).sort((a, b) => a - b);
  if (!claus.length) return 1;
  if (edat <= claus[0]) return taula[String(claus[0])];
  const max = claus[claus.length - 1];
  return taula[String(Math.min(edat, max))] ?? 1;
}
...
// `edats` = bucket → edat que es busca. Els llocs que entrenen la trauen de la corba
// d'entrenament (`edatPerNivell`) i la resta del pom `edat_fitxatge_max`; qui no en porta,
// tarifa sencera.
export function nivellsObjectiu(llocs, cfg, souSostenible, edats = null) {
  const { posicio_aportacio, taula_aportacio, pes_sector, taula_habilitat_lloc, taula_salaris,
    sou_formula, nivell_offset: off = 0, descompte_sou_edat = null } = cfg;
  const comptes = {};
  ...
  for (const lloc of Object.keys(pesos)) {
    ...
    // L'EDAT ENTRA AL PRESSUPOST, no al perfil. El pressupost del lloc és el que pagaràs cada
    // setmana; si el busques de 32, eixe diner compra un perfil de tarifa sencera més gran,
    // perquè al pagar-lo se li aplicarà el descompte.
    const edat = edats?.[lloc] ?? null;
    const desc = descompteEdat(edat, descompte_sou_edat);
    const catalog = pressupost?.[lloc] == null ? null : Math.round(pressupost[lloc] / desc);
    const objectiu = perfilObjectiu(pesosHab, catalog, taula_salaris, sou_formula, { offset: off });
    out[lloc] = {
      pes: pesos[lloc],
      llocs: comptes[lloc],
      edat_objectiu: edat,
      ...
      // EL QUE PAGARÀS DE VERES: el sou de catàleg del perfil ja amb el descompte de l'edat.
      // Així `sobrecost` seguix comparant el sou real d'un jugador amb un sou real.
      sou_objectiu: objectiu?.sou == null ? null : Math.round(objectiu.sou * desc),
...
export async function carregaConfigPesos(db, estrategia) {
  ...
  return {
    ...
    descompte_sou_edat: await constant('descompte_sou_edat'),
    edat_fitxatge_max: Number(await pom('edat_fitxatge_max')) || null,
  };
}
