...
export async function generaAlertes(db, usuariId) {
  ...
  const pla = await db.prepare('SELECT id, plantilla, parametres FROM plans WHERE usuari_id=? LIMIT 1').bind(usuariId).first();
  ...
  const ctx = { jugadors, juvenils, dataInstantania: instSenior.data, any_dies: anyDies, mercat, pla: contextPla, pendents_transaccio: pendents };
