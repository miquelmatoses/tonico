...
export async function generaAlertes(db, usuariId) {
  ...
  const { results: jugadors } = await db.prepare(
    `SELECT ij.jugador_id, j.nom, ij.posicio_ultim_partit AS posicio, ij.edat_anys, ij.edat_dies,
       ...
       LEFT JOIN (SELECT cj.jugador_id, cj.categoria, cj.puntuacio FROM categories_jugador cj
  ...
  if (pla) {
    ...
    // Pròxima eixida de fornada DEL PLA (cadència autoritzada): la temporada futura
    // més pròxima amb eixides + les seues lletres. Serveix per a la finestra de compra.
    const { results: temps } = await db.prepare('SELECT pt.temporada AS t, pt.accions_previstes AS acc FROM plans_temporades pt JOIN plans p ON p.id=pt.pla_id WHERE p.usuari_id=? ORDER BY pt.temporada').bind(usuariId).all();
    let proximaEixida = null;
    for (const tp of temps) {
      if (refTemporada != null && tp.t > refTemporada) {
        const eix = (tp.acc ? JSON.parse(tp.acc).eixides_fornada : null) || [];
        if (eix.length) { proximaEixida = { temporada: tp.t, fornades: eix }; break; }
      }
    }
    contextPla = {
      ...
      supporterCaducitat: params.supporter_caducitat ?? null,
      proxima_eixida: proximaEixida,
  ...
  // Entrenament: prescrit per la fase vs confirmat per l'usuari (Àrea C).
  const planParams = pla?.parametres ? JSON.parse(pla.parametres) : {};
  const entrenamentPrescrit = pla ? await entrenamentFase(db, pla.plantilla, pla.fase_actual) : null;
  ...
  const juvenils = juvenilsRaw.map((f) => ({
    ...
    pipeline: avaluaPipeline(f, entrenamentPrescrit),
  }));
  ...
  if (pla) {
    ...
    const nEntren = jugadors.filter((j) => j.categoria === 'entrenable').length;
    const aforament = config.categories.find((c) => c.categoria === 'entrenable')?.aforament ?? null;
    const nucli_ple = aforament != null && nEntren >= aforament;
    contextCompra = { filtres: filtresCompra(config, jugadors, eco.caixa, compra), caixa: eco.caixa, reserva: parseInt(await par('reserva_operativa', '0'), 10), nucli_ple };
  }
  ...
  if (pla) {
    ...
    if (slotsCfg && slotsCfg.length) {
      ...
      const cob = cobertura({ slots: slotsCfg, rols: rolsCfg }, {
        ...
        marge_absencies: await pomInt('marge_absencies', 2),
      ...
      const cosCamp = jugadors.filter((j) => j.categoria !== 'entrenable' && j.posicio !== porterPos
        && j.transferible !== 1 && !jaLlistats.has(j.jugador_id)).length;
      contextCobertura = { ...cob, cos_camp: cosCamp };
    }
  }
  ...
  // «L'informe és l'agenda d'hui»: els ítems amb acció FUTURA (agenda) no són
  ...
  for (const a of noves) {
    ...
    lots.push(db.prepare(
      `INSERT INTO alertes (usuari_id, regla_id, jugador_id, data, missatge_clau, parametres, estat, urgencia, data_accio)
       VALUES (?, ?, ?, ?, ?, ?, 'nova', ?, ?)`
    ).bind(usuariId, codiToId.get(a.regla_codi), a.jugador_id, instSenior.data, a.missatge_clau, JSON.stringify(a.parametres), a.urgencia ?? 0, a.data_accio ?? null));
    n++;
  }
  ...
  for (const a of novesAgenda) {
    lots.push(db.prepare(
      `INSERT INTO alertes (usuari_id, regla_id, jugador_id, data, missatge_clau, parametres, estat, urgencia, data_accio)
       VALUES (?, ?, ?, ?, ?, ?, 'agenda', 0, ?)`
    ).bind(usuariId, codiToId.get(a.regla_codi), a.jugador_id, instSenior.data, a.missatge_clau, JSON.stringify(a.parametres), a.data_accio ?? null));
  }
