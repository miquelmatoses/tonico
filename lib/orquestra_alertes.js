...
export async function generaAlertes(db, usuariId) {
  ...
  const { results: jugadors } = await db.prepare(
    `SELECT ij.jugador_id, j.nom, ij.posicio_ultim_partit AS posicio, ij.edat_anys, ij.edat_dies,
            ij.porteria, ij.sou, ij.tsi, ij.forma, ij.data_ultim_partit, ij.transferible, c.categoria
  ...
  const ctx = { jugadors, juvenils, dataInstantania: instSenior.data, any_dies: anyDies, mercat, pla: contextPla, pendents_transaccio: pendents, personal: contextPersonal, compra: contextCompra, forma_minima: formaMinima };
