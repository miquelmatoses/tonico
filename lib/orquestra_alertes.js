...
import { diesRestants } from './personal_v3.js';
import { desertsDesats } from './vendes.js';
import { onzeEstructura } from './onze_estructura.js';
import { plaPersonal } from './pla_personal.js';
import { estatEstoc } from './orquestra_estoc.js';
import { setmanaDeHui, carregaAncora, iniciDeSetmana } from './calendari.js';
import { situacioMercat } from './mercat.js';
import { economia } from './economia.js';
import { configHashComplet } from './config_hash.js';
import { entrenamentPrescrit as prescripcio } from './entrenament_places.js';
import { revelacions } from './juvenil.js';
import { plaJuvenilComplet } from './orquestra_juvenil.js';
import { cobertura, cosDisponible } from './cobertura.js';
...
// `hui` = el dia DE VERES. Cal com a paràmetre i no com a `new Date()` amagat perquè
// l'avaluador no pot tindre rellotges propis (els tests han de poder fixar-lo), i cal que siga
// HUI i no la data de l'última instantània: si es compara la frescor de les dades contra la
// data de l'última pujada, quan Miquel deixa de pujar el rellotge es congela i l'avís de dades
// velles no salta MAI — justament quan més falta fa.
export async function generaAlertes(db, usuariId, hui = new Date().toISOString().slice(0, 10)) {
  ...
  const { results: jugadors } = await db.prepare(
    `SELECT ij.jugador_id, j.nom, ij.posicio_ultim_partit AS posicio, ij.edat_anys, ij.edat_dies,
            ...
            ij.experiencia, ij.lideratge,
            ij.bonificacio_origen, ij.lesio, ij.amonestacions
  ...
  if (equipJuvenil) {
    ...
    const filesDe = (id) => db.prepare(
      `SELECT ij.*, j.nom, j.especialitat, j.id AS jugador_id FROM instantanies_juvenils ij
  ...
  // EL RELLOTGE ÉS HUI. La setmana de la instantània diu de quan és el fitxer; la finestra de
  // mercat i la temporada en curs les mou el calendari, encara que no pugeu res en dos setmanes.
  const ara = await setmanaDeHui(db, hui);
  const refTemporada = ara.temporada;
  let mercat = null;
  let contextPla = null;
  if (pla) {
    ...
    if (cal.length && ara.crua.setmana != null) {
      mercat = situacioMercat(cal, ara.crua.setmana, tempSetmanes, parseInt(espera?.valor || '4', 10));
    }
    const params = pla.parametres ? JSON.parse(pla.parametres) : {};
    contextPla = {
      temporadaActual: refTemporada,
      temporada_raw: ara.crua.temporada,            // crua (per a la finestra de compra, alineada amb la setmana de mercat)
  ...
  // Les caducitats (dades velles, números d'estadi) es mesuren contra HUI, no contra la data
  // de la instantània: vore la nota de la signatura.
  const eco = pla ? await economia(db, usuariId, hui) : null;
  ...
  const setmanaIniciData = anc.data ? iniciDeSetmana(hui, anc) : null;
  ...
  // Els DESERTS no generen cap missatge de mercat: ningú els vol i del mercat ja no es parla.
  const deserts = await desertsDesats(db, usuariId);

  // EL GRUP DE CADA JUGADOR, de la MATEIXA font que la pantalla. Abans les regles filtraven per
  // `categoria` —la classificació del PAS 6, que decidia qui es quedava abans de saber qui
  // ocupa cada lloc— i la pantalla ja no la gasta: Paco parlaria d'uns grups i la pantalla en
  // pintaria uns altres. Ara els dos llegixen `onzeEstructura`.
  const est = await onzeEstructura(db, usuariId, jugadors, eco?.sou_sostenible_setmanal ?? null, !!eco?.calibrat);
  for (const j of jugadors) j.grup = est?.grups.get(j.jugador_id) ?? null;
  ...
  // EL CONTEXT JUVENIL IX DEL PLA DEL PAS 10, la MATEIXA font que la pantalla. Abans ací es
  // tornava a avaluar pel seu compte (`avaluaPipeline`, amb els seus llindars cablejats), o
  // siga que Paco i la secció de Juvenils podien dir coses distintes de la mateixa gent.
  const plaJuv = pla && juvenilsRaw.length
    ? await plaJuvenilComplet(db, usuariId, juvenilsRaw, pla.plantilla) : null;
  const sobrenJuv = new Set(plaJuv?.despatxa ?? []);
  const promocionen = new Set(plaJuv?.promocionables ?? []);
  const juvenils = juvenilsRaw.map((f) => ({
    jugador_id: f.jugador_id, nom: f.nom, dies_restants_promocio: f.dies_restants_promocio,
    especialitat: f.especialitat ?? null,
    sobra: sobrenJuv.has(f.jugador_id), promociona: promocionen.has(f.jugador_id),
  }));
  const juvenilObjectiu = plaJuv?.objectiu ?? null;

  // Context de personal: membres declarats (sou, contracte) + desquadre amb la fase.
  let contextPersonal = null;
  if (pla) {
    const { results: membres } = await db.prepare('SELECT rol, tipus, nivell, sou, data_fi_contracte FROM personal_membres WHERE usuari_id=?').bind(usuariId).all();
    // Les setmanes que queden es DERIVEN de la data i d'HUI. Abans era un compte declarat que
    // ningú decrementava: es quedava congelat i l'avís de venciment no arribava mai.
    for (const m of membres) m.dies_contracte = diesRestants(m.data_fi_contracte, hui);
    // FORA els «desquadres» contra `fases_config`: era el personal esperat per FASE del model
    // fàbrica, i la fase real no lligava amb cap fila — es comparava contra el no-res. Qui diu
    // ara què falta és el pla de flux (PAS 11), amb la quota del joc i el pressupost.
    // El PLA de personal entra al context: si hi ha una plaça lliure de la quota, ix com a
    // alerta urgent i no només dins de la seua secció.
    contextPersonal = { membres, pla: (await plaPersonal(db, usuariId, eco))?.pla ?? [] };
  }

  // EL BUCLE D'ESTOC, una sola vegada. D'ací ixen les DUES coses que necessiten les regles:
  // què fa falta fitxar (`mancances`, que és exactament la llista que pinta Mercat) i si
  // l'obra d'estadi és admissible. Abans es cridava al final i, a més, ací es tornava a
  // derivar la llista de necessitats pel seu compte: dues derivacions de la mateixa cosa i
  // una economia i una assignació d'estructura calculades dues voltes per revisió.
  const estoc = pla && eco
    ? await estatEstoc(db, usuariId, hui, { eco, est }).catch(() => null) : null;

  const opEstadi = estoc?.opcions?.find((o) => o.tipus === 'estadi');
  const contextEstoc = opEstadi
    ? { estadi_admissible: opEstadi.admissible, estadi_caduc: opEstadi.caduc,
        estadi_cost: opEstadi.cost, estadi_delta_manteniment: opEstadi.delta_manteniment } : null;

  // «nucli ple» = no falta cap plaça d'entrenament. Abans era sempre false (l'aforament era
  // null a codi), o siga que la condició no feia res.
  const contextCompra = estoc?.mancances
    ? { necessitats: estoc.mancances, caixa: eco?.caixa ?? null,
        nucli_ple: !estoc.mancances.some((n) => n.tipus === 'entrenable') } : null;
  ...
  if (pla) {
    ...
    if (slotsCfg && slotsCfg.length) {
      ...
      const teFuturEntrenador = !!Number((await db.prepare("SELECT valor FROM plantilles_parametres WHERE plantilla=? AND clau='te_futur_entrenador'").bind(pla.plantilla).first())?.valor);
      ...
      // LA MATEIXA VARA que la secció de Vendes: el SOBRECOST, calculat una sola vegada a
      // l'assignació d'estructura. Les dues bandes han de retindre exactament els mateixos
      // jugadors, i abans cada una en tenia una còpia.
      for (const j of jugadors) j.valor = est?.sobrecosts.get(j.jugador_id) ?? 0;
      // QUI NO ENTRENA és cos disponible, i el compte és el MATEIX que fa Vendes: una sola
      // funció, perquè les dues bandes han de retindre exactament els mateixos jugadors.
      const cos = cosDisponible(jugadors, est?.entrenen ?? new Set(),
        { posicio_porter: porterPos, llistats: new Set(vendesLlistades.map((v) => v.jugador_id)) });
      contextCobertura = { ...cob, ...cos, posicio_porter: porterPos };
    }
  }

  const ctx = { estoc: contextEstoc, jugadors, juvenils, dataInstantania: instSenior.data, any_dies: anyDies, porter_notable_min: porterNotableMin, mercat, pla: contextPla, personal: contextPersonal, compra: contextCompra, forma_minima: formaMinima, economia: eco, entrenament: entrenamentPrescrit, juvenil_objectiu: juvenilObjectiu, cobertura: contextCobertura, revelacions: revelacionsJuv, dies_subhasta: diesSubhasta, setmana_inici_data: setmanaIniciData, vendes_llistades: vendesLlistades, deserts };
  ...
  for (const a of noves) {
    ...
    if (prev) {
      // TOT el contingut DERIVAT es refresca, `diners` i `compte` inclosos. Estos dos van
      // arribar després (migracions 068 i 070) i este UPDATE no es va actualitzar: una alerta
      // que ja existia es quedava amb els dos a NULL per sempre, i llavors la vista no podia
      // ni triar la forma de plural (→ «[text no disponible]») ni pintar l'import com a diners
      // (invariant 12). Ací només es preserva l'ESTAT, que és l'únic que és de l'usuari.
      lots.push(db.prepare(
        'UPDATE alertes SET missatge_clau=?, parametres=?, urgencia=?, data=?, diners=?, compte=? WHERE id=?'
      ).bind(a.missatge_clau, JSON.stringify(a.parametres), a.urgencia ?? 0, instSenior.data,
        a.diners ? JSON.stringify(a.diners) : null, a.compte ?? null, prev.id));
      continue;                                    // preserva l'estat (vista/ignorada)
    }
    lots.push(db.prepare(
      `INSERT INTO alertes (usuari_id, regla_id, jugador_id, data, missatge_clau, parametres, estat, urgencia, data_accio, diners, compte)
       VALUES (?, ?, ?, ?, ?, ?, 'nova', ?, ?, ?, ?)`
    ).bind(usuariId, codiToId.get(a.regla_codi), a.jugador_id, instSenior.data, a.missatge_clau, JSON.stringify(a.parametres), a.urgencia ?? 0, a.data_accio ?? null, a.diners ? JSON.stringify(a.diners) : null, a.compte ?? null));
    n++;
  }
  ...
  for (const a of novesAgenda) {
    lots.push(db.prepare(
      `INSERT INTO alertes (usuari_id, regla_id, jugador_id, data, missatge_clau, parametres, estat, urgencia, data_accio, diners, compte)
       VALUES (?, ?, ?, ?, ?, ?, 'agenda', 0, ?, ?, ?)`
    ).bind(usuariId, codiToId.get(a.regla_codi), a.jugador_id, instSenior.data, a.missatge_clau, JSON.stringify(a.parametres), a.data_accio ?? null, a.diners ? JSON.stringify(a.diners) : null, a.compte ?? null));
  }
