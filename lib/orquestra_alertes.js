...
import { carregaConfigPesos } from './pesos.js';
import { necessitats, ambPreus } from './fitxatges.js';
import { pesosFormacio } from './pesos.js';
...
import { avaluaPipeline } from './juvenils_vista.js';
...
export async function generaAlertes(db, usuariId, hui = new Date().toISOString().slice(0, 10)) {
  ...
  const est = await onzeEstructura(db, usuariId, jugadors, eco?.sou_sostenible_setmanal ?? null);
  ...
  // CONTEXT DE COMPRA: les mateixes NECESSITATS que pinta Mercat, no una segona derivació.
  // Abans això comptava categories del PAS 6 amb `filtresCompra` mentres la pantalla mirava
  // l'assignació d'estructura: dues llistes que podien discrepar sobre què falta.
  let contextCompra = null;
  if (pla && est) {
    const cfgP = await carregaConfigPesos(db, pla.plantilla);
    const pesosB = pesosFormacio([...new Set(est.onze.map((l) => l.bucket))],
      cfgP.posicio_aportacio, cfgP.taula_aportacio, cfgP.pes_sector);
    const par = async (clau) => (await db.prepare('SELECT valor FROM plantilles_parametres WHERE plantilla=? AND clau=?').bind(pla.plantilla, clau).first())?.valor ?? null;
    const llista = necessitats(est, { entrenable_min: Number(await par('entrenable_creativitat_min')), pesos: pesosB });
    const { results: fPreu } = await db.prepare('SELECT clau, preu, data FROM preus_referencia WHERE usuari_id=?').bind(usuariId).all();
    const amb = ambPreus(llista, new Map(fPreu.map((f) => [f.clau, f])), eco?.caixa ?? null,
      Number(await par('setmanes_caducitat_preu')) || null, hui);
    ...
    contextCompra = { necessitats: amb, caixa: eco?.caixa ?? null,
      nucli_ple: !amb.some((n) => n.tipus === 'entrenable') };
  }
  ...
  if (pla) {
    ...
    if (slotsCfg && slotsCfg.length) {
      ...
      // VALOR DE VENDA DERIVAT: la MATEIXA vara que usa la secció de Vendes —l'ÚNICA
      // puntuació de venda—, perquè les dues bandes retinguen els MATEIXOS jugadors. Abans
      // cada jugador es mesurava amb la fórmula de la seua CATEGORIA vella; ara no hi ha
      // categories, i mesurar onze jugadors amb varetes distintes no ordenava res de tot.
      const cfgPla = await carregaConfigPla(db, pla.plantilla);
      const vendaSpec = cfgPla.categories.find((c) => c.categoria === 'venda')?.parametres?.puntuacio;
      for (const j of jugadors) j.valor = vendaSpec ? avaluaPuntuacio(vendaSpec, j, cfgPla.params) : 0;
  ...
  // L'estat del bucle d'ESTOC: si l'obra d'estadi és admissible, va dalt de tot (PAS 8).
  let contextEstoc = null;
  if (pla && eco) {
    const e = await estatEstoc(db, usuariId, hui).catch(() => null);
    const op = e?.opcions?.find((o) => o.tipus === 'estadi');
    if (op) contextEstoc = { estadi_admissible: op.admissible, estadi_caduc: op.caduc,
      estadi_cost: op.cost, estadi_delta_manteniment: op.delta_manteniment };
  }
