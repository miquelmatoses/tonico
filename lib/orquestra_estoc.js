...
import { guanyJugador, deltaFlux, guanyEstadi, admissibleEstadi, eficiencia, decisioEstoc } from './estoc.js';
...
export async function estatEstoc(db, usuariId) {
  ...
  // PAS 4 + PAS 5: quin nivell sosté cada lloc i quant li falta al seu ocupant.
  const nivells = nivellsObjectiu([...new Set(llocs.map((l) => l.bucket))], cfg, eco.sou_sostenible);
  ...
  // ── OPCIÓ JUGADOR, per lloc amb mancança. El «cost» és el preu que caldria pagar; sense
  // candidat concret encara, s'usa el preu de referència de la divisió com a escala.
  const basePreu = JSON.parse((await db.prepare("SELECT valor FROM constants_joc WHERE clau='base_preu_divisio'").first())?.valor || '{}');
  const preuRef = eco.divisio ? basePreu[eco.divisio] ?? null : null;
  const opcions = [];
  for (const p of prioritats) {
    ...
    opcions.push({
      ...
      pressupost_sou: p.pressupost_sou, guany, cost,
      eficiencia: eficiencia(guany, cost),
      admissible: cost != null && eco.caixa_disponible != null && cost <= eco.caixa_disponible,
      motiu: 'mancanca',
    });
  }

  // ── OPCIÓ ESTADI, si l'usuari ha declarat els números de la calculadora.
  const fin = await db.prepare('SELECT estadi_manteniment, estadi_cost_obra FROM finances WHERE usuari_id=?').bind(usuariId).first();
  let estadi = null;
  if (fin?.estadi_manteniment != null && fin?.estadi_cost_obra != null) {
    const dFlux = deltaFlux(eco.despeses.manteniment_estadi, fin.estadi_manteniment);
    const pesos = Object.fromEntries(Object.entries(nivells).map(([k, v]) => [k, v.pes]));
    const guany = guanyEstadi(manc, { sou_sostenible: eco.sou_sostenible, delta_flux: dFlux,
      taula_salaris: cfg.taula_salaris, pesos });
    estadi = {
      tipus: 'estadi', delta_flux: dFlux, guany, cost: fin.estadi_cost_obra,
      eficiencia: eficiencia(guany, fin.estadi_cost_obra),
      admissible: admissibleEstadi({ cost: fin.estadi_cost_obra, caixa_disponible: eco.caixa_disponible,
        flux: eco.flux, delta_flux: dFlux }),
      motiu: 'delta_flux',
    };
    opcions.push(estadi);
  }
  ...
  return {
    caixa_disponible: eco.caixa_disponible,
    sou_sostenible: eco.sou_sostenible,
    flux: eco.flux,
    divisio: eco.divisio,
    preu_referencia: preuRef,
    mancances: prioritats,
    opcions: opcions.sort((a, b) => (b.eficiencia ?? -1) - (a.eficiencia ?? -1)),
