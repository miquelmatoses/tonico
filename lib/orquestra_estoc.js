...
export async function estatEstoc(db, usuariId, hui = null, prev = {}) {
  ...
  const llista = necessitats(est, { entrenable_min: Number(await pom('entrenable_creativitat_min')),
    distancia_min: Number(await pom('distancia_min')) || null });
  ...
  const obraMinima = Number(await pom('estadi_obra_minima')) || null;
  const opcions = [];
  let estadi = null;
  if (eco.estadi_manteniment != null && eco.estadi_cost_obra != null) {
    ...
    estadi = {
      ...
      admissible: admissibleEstadi({ cost: eco.estadi_cost_obra, caixa: eco.caixa,
        ...
        obra_minima: obraMinima }),
      // I ES DIU PER QUÈ no es recomana: una obra que no arriba al pas mínim no és que no la
      // pugues pagar, és que no val la pena obrir el rasant per a això.
      motiu: eco.estadi_obra_inici != null ? 'obra_en_curs'
        : (obraMinima != null && eco.estadi_cost_obra < obraMinima) ? 'obra_menuda'
          : 'prioritat_estadi',
    };
    opcions.push(estadi);
  }
  ...
  for (const p of prioritats) {
    ...
    for (let i = 1; i <= (p.quants ?? 1); i++) {
      ...
      opcions.push({
        tipus: 'jugador', clau: p.clau, bucket: p.bucket, perfil: p.perfil, nivell: p.nivell, edat: p.edat,
