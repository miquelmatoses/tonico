...
// Lesió del CSV: la font porta EXACTAMENT dos valors observats — buit (cap lesió) o N
// (lesionat N setmanes). Es mapa NOMÉS això; qualsevol altre valor es registra i la
// cel·la degrada (no s'inventa significat). Vore ENTRADES_MANUALS: l'estat «dolorit»
// (creueta a HT) NO és derivable d'este export.
const lesioCsv = (s) => {
  const v = (s ?? '').trim();
  if (v === '' || v === '0') return null;
  if (/^\d+$/.test(v)) return v;                 // N setmanes de baixa
  console.warn(`[ingest] valor de lesió inesperat, es degrada (no s'inventa): «${v}»`);
  return null;
};
  ...
  return {
    ...
    jugadors: cos.map((c) => ({
      ...
      instantania: {
        ...
        lesio: lesioCsv(c[7]), amonestacions: enter(c[8]),
  ...
  return {
    ...
    jugadors: cos.map((c) => ({
      ...
      instantania: {
        ...
        lesio: lesioCsv(c[5]), amonestacions: enter(c[6]),
