...
// seua secció).
import { planPersonal, decisioRenovacio, diesRestants, nivellPagable, placesAdmeses, sostrePersonal } from './personal_v3.js';
import { costFlux as costPlaça } from './personal_v3.js';
...
export async function plaPersonal(db, usuariId, ecoPrev = null) {
  ...
  // EL QUE JA ES PAGA, abans de decidir res. Una acció es mesura contra el que QUEDA, no
  // contra la simulació: el pla diu que el flux sosté 4 places a nivell 1 (4 080 ≤ 6 564), però
  // eixe món suposa que els tres que ja hi són han baixat de nivell 2 a 1 — i no poden fer-ho
  // fins al venciment. Hui se'n paguen 6 120, o siga que contractar-ne un altre val 7 140 i NO
  // es pot. Proposar-ho era proposar una cosa que no es pot pagar.
  const declarats = membres.reduce((a, m) => a + (m.sou ?? 0), 0);
  let lliure = pressupost - declarats;               // el que queda de veres, ara mateix
  ...
  const amb = pla.map((x) => {
    ...
    if (d == null) {
      // Plaça lliure: el nivell més alt que el que QUEDA paga, i mai per damunt del que el pla
      // sosté a llarg (si no, es contractaria hui un nivell que al venciment tocaria baixar).
      const n = Math.min(nivellPagable(lliure, base, nivellMax), x.nivell);
      if (n > 0) { accio = 'contracta'; nivellAccio = n; lliure -= costPlaça(n, base); }
    } else if (venç) {
      // Al venciment, el seu propi sou torna al calaix: eixe és el pressupost d'esta plaça.
      const seu = lliure + (d.sou ?? 0);
      const n = Math.min(nivellPagable(seu, base, nivellMax), x.nivell);
      if (n > (d.nivell ?? 0)) { accio = 'renova_al_nivell'; nivellAccio = n; }
      else {
        const r = decisioRenovacio(d.nivell, seu, base, nivellMax);
        accio = r.accio; nivellAccio = r.nivell;
      }
      lliure = seu - (nivellAccio ? costPlaça(nivellAccio, base) : 0);
    }
