...
export function preguntaVenda(jugador) {
  const eraLlistat = jugador?.transferible_anterior === 1 || jugador?.estat_anterior === 'llistat';
  const jaNoHoEs = !(jugador?.transferible === 1) && jugador?.estat !== 'llistat';
  if (!eraLlistat || !jaNoHoEs) return null;
  if (jugador?.venda_apuntada) return null;              // ja sabem què va passar
