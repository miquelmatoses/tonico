...
export function alinea(squad, config, opts = {}) {
  ...
  // 1. MC entrenables repartits (els més pròxims a la venda → lliga)
  const mcs = disp.filter((p) => p.categoria === 'entrenable' && bucketDe(p) === 'mc')
    .sort((a, b) => (a.fornada_eixida ?? 99) - (b.fornada_eixida ?? 99));
  const perMatch = nSlots('mc');
  mcs.forEach((p, i) => posa(i < perMatch ? partits[0] : partits[1], 'mc', p));

  // 2. Extrems entrenables → als dos partits (50%+50%)
  disp.filter((p) => p.categoria === 'entrenable' && bucketDe(p) === 'extrem').slice(0, nSlots('extrem'))
    .forEach((p) => partits.forEach((partit) => posa(partit, 'extrem', p)));
