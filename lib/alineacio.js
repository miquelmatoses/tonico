...
export function alinea(squad, config, opts = {}) {
  ...
  // Cap onze de 10: ompli els buits restants (incloses posicions d'entrenament
  // sense entrenable disponible) amb qualsevol cos. Prefereix la posició natural.
  // La cobertura per un no-entrenable queda reflectida als avisos, no com a forat.
  for (const partit of partits) {
    for (const slot of onze[partit].filter((s) => !s.jugador)) {
      const cos = disp.find((p) => !usat.has(`${partit}:${p.jugador_id}`) && p.categoria !== 'entrenable' && bucketDe(p) === slot.bucket)
        || disp.find((p) => !usat.has(`${partit}:${p.jugador_id}`) && p.categoria !== 'entrenable');
