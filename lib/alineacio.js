...
//
// squad: [{jugador_id, nom, posicio, categoria, horitzo_eixida, lesionat, suspes,
//          llistat}]
...
// Qui entrena (contracte v3): el core i els rotatius que el doblen.
const ENTRENEN = new Set(['core', 'rotatiu']);
...
export function alinea(squad, config, opts = {}) {
  ...
  const entrenables = disp.filter((p) => ENTRENEN.has(p.categoria));
  ...
  const doblats = [...entrenables]
    .sort((a, b) => (buckets50.includes(bucketDe(a)) ? 0 : 1) - (buckets50.includes(bucketDe(b)) ? 0 : 1)
      || (a.horitzo_eixida ?? 99) - (b.horitzo_eixida ?? 99))
    .slice(0, n50);
  const setDoblats = new Set(doblats.map((p) => p.jugador_id));
  const simples = entrenables.filter((p) => !setDoblats.has(p.jugador_id))
    .sort((a, b) => (a.horitzo_eixida ?? 99) - (b.horitzo_eixida ?? 99));  // pròxims a la venda → competitiu
  ...
  const cosPool = disp.filter((p) => !ENTRENEN.has(p.categoria) && p.categoria !== 'futur_entrenador');
...
function comptabilitat(onze, disp, partits) {
  return disp.filter((p) => ENTRENEN.has(p.categoria)).map((p) => {
    const files = [];
    for (const partit of partits) {
...
function avisos(onze, squad, partits, vetats, unaAlineacio) {
  ...
  const entrenables = squad.filter((p) => ENTRENEN.has(p.categoria));
  let entrenen = 0;
