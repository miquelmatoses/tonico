...
export function desquadreEntrenament(prescrit, confirmat) {
  if (!confirmat) return { desquadre: true, motiu: 'sense_confirmar', difs: ['tot'] };
  const difs = ['skill', 'skill_b', 'intensitat', 'resistencia']
    .filter((k) => prescrit[k] != null && String(prescrit[k]) !== String(confirmat[k]));
  return { desquadre: difs.length > 0, motiu: difs.length ? 'difereix' : null, difs };
}
