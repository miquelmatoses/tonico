...
export async function economia(db, usuariId) {
  ...
  const inst = eq ? await db.prepare('SELECT id FROM instantanies WHERE equip_id=? ORDER BY data DESC, id DESC LIMIT 1').bind(eq.id).first() : null;
  ...
  return { caixa, margesFornada, projeccio, nomina };
}
