...
//
// EL PERÍODE ÉS BI-SETMANAL (v3.1). En lliga es juga un partit a casa i un fora, així que la
// taquilla entra en setmanes ALTERNES: al fixture real de Benifotrem, 0 € una setmana i
// 21.127 € l'altra sobre un flux net de milers. Un flux calculat sobre una setmana oscil·la
// més que el propi flux. Només la taquilla ja ve per període; tot el que és setmanal es
// normalitza amb `per_periode` (invariant 16), i l'ÚNIC pas de tornada a unitats setmanals
// és el PAS 4, on es compara amb `taula_salaris`.
//
...
export async function economia(db, usuariId, hui = null) {
  const fin = await db.prepare(
    `SELECT caixa, caixa_data, despesa_estadi, periode_data, taquilla, patrocini,
  ...
  // INGRESSOS RECURRENTS = taquilla (ja per període) + per_periode(patrocini). NOMÉS els dos
  // extrapolables: club d'aficionats, comissions i vendes són PUNTUALS i no entren al flux.
  // Sense taquilla ni patrocini declarats NO hi ha flux, i es diu, en compte de fabricar un 0.
  const taquilla = fin?.taquilla ?? null;
  const patrocini = fin?.patrocini ?? null;
  const ingressosRecurrents = (taquilla == null && patrocini == null)
    ? null
    : (taquilla ?? 0) + perP(patrocini ?? 0);
  ...
  return {
    ...
    periode_data: fin?.periode_data ?? null,
    setmanes_periode: setmanesPeriode,
    dades_velles: dadesVelles(fin?.periode_data, hui, await pom('setmanes_avis_dades', 1)),
    taquilla,
    patrocini,
