...
// entra en setmanes ALTERNES: al fixture real de Sènior FC, 0 € una setmana i 21.127 € l'altra
...
import { carregaAncora, iniciDeSetmana } from './calendari.js';
import { entrenadorSostingut } from './personal_v3.js';
...
// flux_repartible: EL CALAIX ÚNIC del qual ixen els sous del personal I els dels jugadors.
//
//   ingressos − planter − manteniment_estadi − sou_entrenador − reserva
//
// Es lleven els FETS (planter derivat, manteniment declarat, sou de l'entrenador —que només
// canvia si canvies d'entrenador—) i queda el que sí que es decidix. Abans hi havia dos
// bucles que es tractaven mútuament com a fixos: `sou_sostenible` restava el personal i
// `flux_lliure` restava la nòmina, així que cap dels dos s'optimitzava.
export function fluxRepartible(ingressos, fixesPeriode, reserva) {
  if (ingressos == null || reserva == null) return null;
  return Math.max(0, ingressos - (fixesPeriode ?? 0) - reserva);
}

// El pressupost del personal: una QUOTA del repartible, acotada pel que el personal pot
// absorbir de veres. Passat el sostre, el sobrant va als jugadors — no es reserva diner que
// no es pot gastar. La quota és política declarada; el NIVELL n'és conseqüència.
export function pressupostPersonal(repartible, quota, sostre) {
  if (repartible == null || quota == null) return null;
  const p = repartible * quota;
  return sostre == null ? p : Math.min(p, sostre);
}
...
export const UNITATS = {
  ...
  reserva_flux: 'periode', sou_sostenible: 'periode', flux_repartible: 'periode',
  flux_setmanal: 'setmana', mitjana_setmanal: 'setmana',
  'despeses_setmanals.nomina': 'setmana', 'despeses_setmanals.planter': 'setmana',
  'despeses_setmanals.manteniment_estadi': 'setmana', 'despeses_setmanals.personal': 'setmana',
  'despeses.nomina': 'periode', 'despeses.planter': 'periode',
  'despeses.manteniment_estadi': 'periode', 'despeses.personal': 'periode',
  'despeses.entrenador': 'periode', 'despeses_setmanals.entrenador': 'setmana',
  sou_sostenible_setmanal: 'setmana', flux_repartible_setmanal: 'setmana',
  sou_entrenador: 'setmana',
  nomina: 'setmana', planter_derivat: 'setmana',
...
export async function economia(db, usuariId, hui = null) {
  const fin = await db.prepare(
    `SELECT caixa, caixa_data, despesa_estadi,
            ...
            estadi_obra_inici FROM finances WHERE usuari_id=?`
  ).bind(usuariId).first();
  ...
  const personalSetmanal = (await db.prepare(
    "SELECT COALESCE(SUM(sou),0) s FROM personal_membres WHERE usuari_id=? AND rol<>'entrenador'"
  ).bind(usuariId).first()).s;
  ...
  // LES DUES SETMANES QUE ES DECLAREN, i les diu el CALENDARI. Declarar no és triar una data:
  // és transcriure l'informe de HT, que sempre parla d'esta setmana i de la passada. La data
  // era un camp del formulari i era l'única manera d'apuntar diners a la setmana equivocada.
  const ancEco = await carregaAncora(db);
  const huiRef = hui || new Date().toISOString().slice(0, 10);
  const perData = new Map(setmanes.map((x) => [x.data, x]));
  const setmanaDe = (setmanesEnrere) => {
    if (!ancEco.data) return null;
    const dia = new Date(Date.parse(huiRef) - setmanesEnrere * 7 * 86400000).toISOString().slice(0, 10);
    const data = iniciDeSetmana(dia, ancEco);
    return { data, ...(perData.get(data) ?? { taquilla: null, patrocini: null }) };
  };
  const declaracio = { esta: setmanaDe(0), passada: setmanaDe(1) };
  ...
  // El sou de l'entrenador és un FET, com el manteniment: només canvia si canvies d'entrenador.
  // Per això ix del pressupost de personal i entra als fixos (guia «Coach»: no és especialista).
  const souEntrenador = (await db.prepare(
    "SELECT COALESCE(SUM(sou),0) s FROM personal_membres WHERE usuari_id=? AND rol='entrenador'"
  ).bind(usuariId).first()).s;
  const despeses = {
    ...
    personal: perP(personalSetmanal),                       // especialistes; l'entrenador va a banda
    // L'ENTRENADOR HI FALTAVA, i és un sou que es paga cada setmana com qualsevol altre. Sense
    // ell, `flux` i `sou_sostenible` eixien inflats per tot el seu import: amb 5.000 €/setmana,
    // el sostre de sous deia 15.291 quan eren 10.291. I d'eixe sostre pengen els
    // `nivell_objectiu` de TOTS els llocs, o siga l'onze ideal sencer, les mancances, l'orde
    // del PAS 8 i el motiu «sou desproporcionat» del PAS 7: tot dissenyat amb un 49% de més.
    // `flux_repartible` sí que el restava — dues xifres del mateix pas amb criteris distints.
    entrenador: perP(souEntrenador),
  };
  const despesesFixes = despeses.nomina + despeses.planter + despeses.manteniment_estadi
    + despeses.personal + despeses.entrenador;
  ...
  // Els FETS del període: tot menys la nòmina i el personal, que són les dues decisions.
  const fixesPeriode = despeses.planter + despeses.manteniment_estadi + perP(souEntrenador);
  const repartible = fluxRepartible(ingressosRecurrents, fixesPeriode, reserva);
  // L'ENTRENADOR, amb la seua quota. És l'únic element amb cost i efecte coneguts que estava
  // només a la banda dels FETS: es restava sencer i no participava de cap repartiment, o siga
  // que per molt que els ingressos crescueren, mai hi havia un moment de millorar-lo.
  const entrenador = entrenadorSostingut({
    ingressosSetmanals: ingressosRecurrents == null ? null : ingressosRecurrents / setmanesPeriode,
    quota: await pom('quota_entrenador', null),
    base: await pom('entrenador_cost_base', null),
    nivellMax: Number((await db.prepare("SELECT valor FROM constants_joc WHERE clau='entrenador_nivell_max'").first())?.valor) || 5,
    souActual: souEntrenador || null,
  });

  const quotaPersonal = await pom('quota_personal', 0);
  ...
  return {
    ...
    declaracio,                                // les dues setmanes que toca declarar, amb el que ja hi ha
    ...
    flux,
    // TOT EL QUE ES MOSTRA VA EN SETMANES. El model calcula per període perquè la taquilla
    // entra en setmanes alternes, però barrejar les dues unitats a la pantalla marejava.
    // Ací es tanca la conversió, una sola vegada, i la vista ja no dividix mai.
    flux_setmanal: flux == null ? null : flux / setmanesPeriode,
    despeses_setmanals: {
      nomina: despeses.nomina / setmanesPeriode,
      planter: despeses.planter / setmanesPeriode,
      manteniment_estadi: despeses.manteniment_estadi / setmanesPeriode,
      personal: despeses.personal / setmanesPeriode,
      entrenador: despeses.entrenador / setmanesPeriode,
    },
    ...
    // EL CALAIX ÚNIC: es lleven els fets (planter, manteniment, sou de l'entrenador) i queda
    // el que es decidix. D'ací ixen tant els sous del personal com els dels jugadors.
    flux_repartible: repartible,
    flux_repartible_setmanal: repartible == null ? null : repartible / setmanesPeriode,
    quota_personal: quotaPersonal,
    sou_entrenador: souEntrenador,
    // L'escala de personal (`staff_cost_base` × 2^(n−1)) va en €/SETMANA, com la de salaris.
    // El PAS 11 s'hi compara, així que ací es fa el canvi d'unitat — el mateix punt i el mateix
    // criteri que `sou_sostenible_setmanal` al PAS 4. Sense això el pla de personal es pensava
    // que tenia el DOBLE de pressupost i proposava un assistent de nivell 5 (16.320 €/setmana).
    ...
    // Una obra COMENÇADA no és una decisió pendent: mentre dura, l'estadi no es proposa.
    estadi_obra_inici: fin?.estadi_obra_inici ?? null,
