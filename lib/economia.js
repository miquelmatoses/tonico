...
// +, despeses −). Caixa REAL declarada (informe de HT), marges per fornada, balanç
...
export async function economia(db, usuariId) {
  ...
  // Trajectòria fins a la DATA d'inflexió, INCLOENT el negoci (vendes + fornades).
  const traj = await trajectoria(db, usuariId, projeccio, caixa, balancSetmanal, params.temporada_inflexio ?? null, inst, pla?.plantilla, margesFornada);
  if (projeccio && traj) Object.assign(projeccio, traj);

  return { caixa, caixaReal, caixa_data: fin?.caixa_data ?? null, margesFornada, projeccio, nomina,
...
//  · cadència de fornades del pla (valor per fornada, recalibrat amb marges reals).
async function trajectoria(db, usuariId, projeccio, caixa, balancSetmanal, temporadaInflexio, inst, plantilla, margesFornada) {
  ...
  const valorFornadaPom = await pom('valor_fornada_estimat', 0);
  ...
  // (b) Cadència de fornades DEL PLA (plans_temporades): fornades DISTINTES que ixen
  //     abans de la inflexió, valorades amb el pom o RECALIBRADES amb el marge real
  //     mitjà de les ja venudes (punt 1d).
  const { results: temps } = await db.prepare(
    'SELECT pt.temporada AS t, pt.accions_previstes AS acc FROM plans_temporades pt JOIN plans p ON p.id=pt.pla_id WHERE p.usuari_id=?'
  ).bind(usuariId).all();
  const eixint = new Set();
  for (const tp of temps) {
    if (temporadaActual == null || !(tp.t > temporadaActual && tp.t <= temporadaInflexio)) continue;
    const acc = tp.acc ? JSON.parse(tp.acc) : {};
    for (const l of acc.eixides_fornada || []) eixint.add(l);
  }
  const futures = eixint.size;
  const margesReals = (margesFornada || []).filter((m) => m.vendes > 0).map((m) => m.vendes);
  const teVendesReals = margesReals.length > 0;
  const valorFornada = teVendesReals ? Math.round(margesReals.reduce((a, b) => a + b, 0) / margesReals.length) : valorFornadaPom;
  const fornadesPrevistes = futures * valorFornada;

  const ingresEstimat = vendesPrevistes + fornadesPrevistes;
  ...
  return {
    ...
    vendes_previstes: vendesPrevistes, fornades_previstes: fornadesPrevistes, ingres_estimat: ingresEstimat,
    te_comparables: teComparables, te_vendes_reals: teVendesReals,
    sense_dades_venda: !teComparables && !teVendesReals && vendesRows.length === 0,
  };
}
