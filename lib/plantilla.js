...
//   core       = PRIMERS(N_core; ORDENA(hab(j,A) DESC, edat_d ASC))
...
// N_core     = COMPTA(pos_A)                                  ← NO depén de partits_setmana
// N_rotatius = SUMA(pos_A amb pct=100: partits_setmana − 1)    ← sí que en depén
//
// Fer que N_core depenguera de partits_setmana era un afegit que el full no té, i amb el
// PAS 0 incomplet buidava el core sencer.
export function comptesNucli(slots, partitsSetmana) {
  const entrenen = (slots || []).filter((s) => s.entrena);
  const n_core = entrenen.length;
  if (!partitsSetmana) return { n_core, n_rotatius: null };
...
// `opts` = { A, any_dies, partits_setmana, llocs_partit, posicio_porter }
export function construeixPlantilla(plantilla, llocs, opts) {
  const { A, any_dies = 112, partits_setmana,
  ...
  // FORA ELS ROTATIUS. Eren els joves que doblaven els llocs del 100%, triats per una
  // finestra d'edat (`edat_pic_venda`, del model fàbrica). Eixa figura ara són els ENTRENABLES,
  // i no ixen d'una classificació prèvia sinó del residu de l'assignació d'estructura, amb un
  // criteri que el pom d'edat no podia expressar: que la pròxima pujada de nivell els caiga
  // abans del límit. Amb ells se'n va `edat_pic_venda`.
  const rotatius = [];
