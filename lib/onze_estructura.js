...
export async function onzeEstructura(db, usuariId, jugadors, souSostenibleSetmanal = null, calibrat = false) {
  ...
  // que es venen cars quan han crescut. Ixen del VIVER —qui no entrena en l'onze oficial, no
  // d'una categoria decidida abans— i es trien pel mateix criteri que el full ja tenia per
  // als rotatius: dins de la finestra d'edat de venda i pels millors en l'habilitat entrenada.
  ...
  // EL VIVER: qui NO entrena en l'onze oficial i estaria bé que entrenara. El residu, i
  // TAMBÉ els ocupants dels llocs de l'onze que no entrenen — una lesió pot arrossegar un
  // jove a davanter (l'assignació és maximització pura i no posa preu a l'entrenament
  // perdut): seguix sent entrenable, i l'onze_B (PAS 9) el torna a posar a entrenar.
  // El porter no: el porter no dobla.
  const viver = [...sobrants,
    ...onze.filter((l) => l.jugador && !l.entrena && l.bucket !== 'porter').map((l) => l.jugador)];
  const ambDif = (!habA || !creativitatMin || !edatLimit ? [] : viver
    .map(ambCalcul)
  ...
  return { onze, sobrants, aturats, no_disponibles: new Set(nodisponibles.map(id)),
    entrenables: ambDif, habilitat_entrenament: habA, entrenen, sobrecosts,
