...
export async function onzeEstructura(db, usuariId, jugadors, souSostenibleSetmanal = null) {
  ...
  const entrenables = !habA || !edatMax ? [] : sobrants
    ...
    .slice(0, quants);
  // Els entrenables es mesuren contra la MATEIXA vara que els llocs que entrenen. I porten el
  // «10(6)»: el que els costarà el nivell següent i el que els va costar l'actual.
  ...
  const ent = await db.prepare("SELECT nivell FROM personal_membres WHERE usuari_id=? AND rol='entrenador' LIMIT 1").bind(usuariId).first();
  ...
  const ambDif = entrenables.map((j) => {
    ...
    const base = { nivell: Number(j[habA] ?? 0), edat: Number(j.edat_anys ?? 0), habilitat: habA,
      entrenador: ent?.nivell ?? null, assistents: assist, intensitat, resistencia };
