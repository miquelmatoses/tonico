...
export async function onzeEstructura(db, usuariId, jugadors, souSostenibleSetmanal = null) {
  ...
  //    tirar-les. És el mateix llistó amb què es compra (`compra_creativitat_min`).
  ...
  // «Porter» és qui té la porteria ESTRICTAMENT per damunt de la resta. Amb «>=» un jugador
  // amb totes les habilitats iguals —un descart amb tot a 1— comptava com a porter i es
  // quedava la plaça de suplent en compte d'anar-se'n a venda.
  const esPorter = (j) => HABS.filter((h) => h !== 'porteria')
    .every((h) => Number(j.porteria ?? 0) > Number(j[h] ?? 0));
  ...
  return { onze, sobrants, entrenables: ambDif, habilitat_entrenament: habA, entrenen,
