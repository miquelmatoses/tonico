...
import { carregaConfigPesos, pesosFormacio, nivellsObjectiu, equivalent } from './pesos.js';
import { llegixConfig } from './config.js';
import { llocsAmbHabilitat } from './orquestra_estoc.js';
import { desertsDesats } from './vendes.js';
import { carregaCoeficients, setmanesFinsAlSegüent, velocitat, edatPerNivell } from './entrenament_velocitat.js';
...
export async function onzeEstructura(db, usuariId, jugadors, souSostenibleSetmanal = null, calibrat = false) {
  ...
  // ── A QUINA EDAT ES BUSCA CADA LLOC ──────────────────────────────────────────────────
  // No és el mateix comprar per a jugar que comprar per a entrenar.
  //
  //  · Llocs que NO entrenen → `edat_fitxatge_max`, i és POLÍTICA. El sou baixa un 10% per any
  //    des dels 29, o siga que el mateix diner compra molt més amunt: el central passa de
  //    CR4/DF8 a CR8/DF9. El motor a soles sempre diria «el més vell possible» —la caiguda
  //    d'habilitat és molt més lenta que el descompte—, i els tres costos que ho frenarien
  //    (reposar-lo, les lesions, la revenda) són justament els que no modelem.
  //  · Llocs que ENTRENEN → l'edat que trau la CORBA. Ahí no es compra un vell: un que entra
  //    als 17 amb el llistó arriba a ~19,6 de creativitat i el pressupost no passa de 9. Es
  //    busca el MÉS JOVE que ja porte el nivell del perfil, que és el que encara té recorregut.
  ...
  const buckets = llocs.map((l) => l.bucket);
  const entrena = new Map(llocs.map((l) => [l.bucket, !!l.entrena]));
  // Primera passada SENSE edat: fa falta el perfil per a saber quin nivell demana el lloc, i
  // d'ahí ix l'edat dels que entrenen. Als joves no hi ha descompte, o siga que el seu perfil
  // no canvia entre les dues passades: no hi ha cap circularitat.
  const base = nivellsObjectiu(buckets, cfg, souSostenibleSetmanal);
  const edats = {};
  for (const [bucket, v] of Object.entries(base)) {
    const hab = entrena.get(bucket) ? (habA ?? v.habilitat) : null;
    edats[bucket] = hab && coefs && creativitatMin
      ? edatPerNivell(coefs, { habilitat: hab, entrenador: ent?.coach_entrenament ?? null,
        assistents: assist, intensitat, resistencia },
      v.perfil_objectiu?.[hab] ?? null, { nivellInicial: creativitatMin })
      : (cfg.edat_fitxatge_max ?? null);
  }
  const nivells = nivellsObjectiu(buckets, cfg, souSostenibleSetmanal, edats);
  const ambPes = llocs.map((l) => ({ ...l, pes: pesos[l.bucket] ?? 0,
    // ELS PESOS PER HABILITAT del lloc: amb ells, la tria i la mesura miren la contribució
    // sencera i no una sola habilitat (v2 del motor).
    pesos_habilitat: nivells?.[l.bucket]?.pesos_habilitat ?? null,
    perfil_objectiu: nivells?.[l.bucket]?.perfil_objectiu ?? null,
    edat_objectiu: nivells?.[l.bucket]?.edat_objectiu ?? null,
    // I RES MÉS. Ací hi havia `nivell_objectiu`: l'equivalent del perfil, un sol número que
    // aplanava les quatre habilitats. Era l'últim rastre del monocultiu — deixava passar per
    // «aliniat» un mig centre amb creativitat 12 i defensa 1. El que mesura és el perfil.
    sou_objectiu: nivells?.[l.bucket]?.sou_objectiu ?? null }));
  // ── QUI NO POT JUGAR ─────────────────────────────────────────────────────────────────
  ...
  // ── EL QUE UN JOVE GUANYA OCUPANT UN LLOC QUE ENTRENA ────────────────────────────────
  // Un lloc que entrena no val el mateix segons qui l'ocupe: al que encara creix li torna
  // habilitat cada setmana, i al que ja no, res. Sense comptar-ho, l'assignació donava el lloc
  // d'extrem a un jugador de 32 anys —millor HUI— i es menjava una plaça d'entrenament que no
  // li tornaria mai res; el que sobreeixia era un de 20 que sí que creixia.
  //
  // LA UNITAT ÉS UNA SETMANA, i és la mínima possible a posta. L'onze es torna a calcular a
  // cada pujada, o siga que ocupar el lloc HUI et dona exactament una setmana d'entrenament:
  // apuntar-se'n una temporada sencera seria donar per fet que el conserva fins al final, cosa
  // que el model no garantix. Amb la setmana el plus només desfà empats i quasi-empats —que és
  // on el problema estava— i costa un 0,03% d'aportació en compte d'un 0,75%.
  //
  // NO CAL CAP TALL D'EDAT: `velocitat` ja baixa amb els anys, o siga que el plus d'un vell és
  // menut tot sol i el d'un jove gran. Amb el plus d'una temporada sí que en calia un —un vell
  // acumulava massa—, però amb el d'una setmana la corba ja fa eixa feina, i un tall que no
  // canvia cap resultat és una branca que ningú pot provar.
  const plusEntrenament = (j, l) => {
    if (!l.entrena || !coefs || !habA || !l.pesos_habilitat) return 0;
    const w = l.pesos_habilitat;
    const suma = Object.values(w).reduce((a, b) => a + b, 0);
    if (!suma || !w[habA]) return 0;
    const minuts = (l.pct ?? 100) === 100 ? 90 : Math.round(90 * (l.pct ?? 100) / 100);
    const base = { habilitat: habA, entrenador: ent?.coach_entrenament ?? null, assistents: assist,
      intensitat, resistencia, minuts, edat: Number(j.edat_anys ?? 0) };
    const T = velocitat(coefs, { ...base, nivell: Number(j[habA] ?? 0) });
    if (!T) return 0;
    return T * w[habA] / suma;
  };

  const ideal = assignaEstructura(jugadors, ambPes, plusEntrenament);
  ...
  const real = nodisponibles.length ? assignaEstructura(jugadors.filter(disponible), ambPes, plusEntrenament) : ideal;
  ...
  for (const j of jugadors) {
    ...
    // mateixa vara amb què se'l tria. I el que es compara és el sou del PERFIL que eixe lloc
    // paga, o siga el que costaria de veres un jugador fet a mida per al lloc.
    const seu = perBucket
      .map(([, n]) => ({ n, ap: equivalent(j, n.pesos_habilitat) ?? 0 }))
      .sort((a, b) => b.ap - a.ap)[0];
    sobrecosts.set(id(j), sobrecost(j, seu.n.sou_objectiu) ?? 0);
  }
  ...
  // L'ENTRENAMENT SÍ QUE ÉS D'UNA HABILITAT, o siga que ací el número d'una sola és el correcte
  // — però ha de ser el del PERFIL, no l'equivalent aplanat del lloc. El que li demanes a un
  // jove és «creativitat 9», que és el que el perfil del mig centre porta a eixa habilitat.
  const objectiuA = plens[0] && habA ? (nivells?.[plens[0].bucket]?.perfil_objectiu?.[habA] ?? null) : null;
