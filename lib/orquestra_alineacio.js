...
export async function proposaAlineacio(db, usuariId, opts = {}) {
  ...
  const { results: files } = await db.prepare(
    `SELECT ij.jugador_id, j.nom, ij.posicio_ultim_partit AS posicio, ij.lesio, ij.amonestacions, ij.transferible,
            c.categoria, ij.edat_anys, ij.edat_dies
...
function avisosOnze(res, squad, llocs, unaAlineacio = false) {
  ...
  for (const j of squad.filter((x) => NUCLI.has(x.categoria))) {
    ...
    const motiu = j.lesionat ? 'lesionat' : j.llistat ? 'llistat' : j.suspes ? 'sancionat'
