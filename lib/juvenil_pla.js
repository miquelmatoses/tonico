...
// La CUA: els que no han entrat en cap passada. Mana el PRODUCTE —la creativitat—, amb el
// MATEIX conveni que el tall: qui SABEM que no hi arriba baixa, i el desconegut val el llistó.
//
// Abans manava «la millor habilitat revelada» a seques, i això feia el contrari del que diu
// la doctrina: un capat a 4 amb anotació 5 passava davant d'un misteri amb tot el recorregut
// per davant, i com que despatxar és l'orde invers, el que se n'anava era el misteri. Un
// potencial enorme en una habilitat que no s'entrena no mou cap decisió — tampoc esta.
//
// La millor habilitat revelada es queda de DESEMPAT, per als que ni tan sols això en diu res.
...
export function plaJuvenil(juvenils, opts) {
  const { passades = [], places = {}, habs = [], estreles = {}, minimEnCamp = 9,
    residuals = [] } = opts;
  ...
  // La primera passada és la PRINCIPAL (el producte): la seua avaluació és la vara de la cua.
  const principal = passades[0] ?? null;
  const valorPrincipal = (j) => (principal
    ? avaluaHabilitat(j, principal.habilitat, principal).valor : -1);
  const cua = lliures
    .map((j) => ({ j, val: valorPrincipal(j), max: maxConegut(j, habs) }))
  ...
  // I EIXIR AMB UNA POSICIÓ, no amb un «residual» genèric. Si la plantilla és roïna i no
  // entrena ningú —tots capats per davall del llistó— açò és l'única cosa que li diu a Miquel
  // com plantar els nou al camp, i «–» no li diu res: ha de vore que hi ha un porter i quants
  // defenses posa. L'ORDE és declarat (`orde_alineacio_residual`): porter, defenses, davanters.
  //
  // Les places que les passades d'entrenament ja han ocupat no es tornen a repartir.
  let falten = Math.max(0, minimEnCamp - assignats.size);
  let i = 0;
  for (const bucket of residuals) {
    const lliuresAcí = (places[bucket] ?? 0) - (ocupades.get(bucket) ?? 0);
    for (let k = 0; k < lliuresAcí && falten > 0 && i < cua.length; k++, i++, falten--) {
      assignats.set(cua[i].jugador_id, bucket);
      ocupades.set(bucket, (ocupades.get(bucket) ?? 0) + 1);
    }
  }
  const banqueta = cua.slice(i);
