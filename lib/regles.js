...
export const REGLES = {
  ...
  ALR_ANIVERSARI: (ctx, p) => {
    // Pel GRUP, no per la categoria: un aniversari importa dels que et quedes (l'onze i els
    // entrenables), i als que se'n van els puja el sou però ja no és el teu problema.
    const cats = p.categories ? String(p.categories).split(',') : ['onze', 'entrenable'];
    // Els que se'n van no esperen aniversari, encara que el pom els nomene: pujar-los el sou
    // no és un problema teu si ja estan al mercat. Ho governa ALR_LLISTAR_VENDA.
    const SURT = new Set(['venda', 'despatxar']);
    return ctx.jugadors
      .filter((j) => cats.includes(j.grup) && !SURT.has(j.grup))
  ...
  ALR_LLISTAR_VENDA: (ctx, p) => {
    ...
    const venda = (ctx.jugadors || [])
      .filter((j) => j.grup === 'venda' && j.transferible !== 1 && j.transferible !== true
  ...
  ALR_NUCLI_INCOMPLET: (ctx, p) => {
    ...
    const n = ctx.jugadors.filter((j) => j.grup === 'onze' || j.grup === 'entrenable').length;
  ...
  // Juvenil promocionable a la vora. NO és una decisió —es promociona el primer dia possible
  // (PAS 10)—, o siga que això és un avís d'agenda: prepara't, que la setmana que ve el tens
  // al primer equip i ocuparà plaça d'entrenable.
  ...
  // QUI SOBRA A L'ACADÈMIA. Una sola línia agregada; el detall per jugador viu a la secció.
  //
  // Ix del PLA (`sobra`), no d'una segona derivació. Abans ací hi havia DUES regles que deien
  // la mateixa cosa per camins distints —una comptava «fora de pipeline» amb els llindars
  // cablejats de `avaluaPipeline`, l'altra cridava `ranquingEixida`— i cap de les dues sabia
  // el que la pantalla mostrava.
  ALR_JUVENIL_SOBRANT: (ctx, p) => {
    const sobren = (ctx.juvenils || []).filter((j) => j.sobra);
    if (!sobren.length) return [];
    return [alerta('ALR_JUVENIL_SOBRANT', null, 'alerta.juvenil_sobrant',
      { sobren: sobren.length, objectiu: ctx.juvenil_objectiu ?? 0,
        noms: sobren.map((j) => j.nom).join(', ') }, p.urgencia, null, null, 'sobren')];
  },

  // Un juvenil amb ESPECIALITAT descoberta que està a punt d'eixir: val la pena mirar-lo dues
  // voltes abans de despatxar-lo. Només si de veres sobra — protegir algú que ningú pensa
  // traure no és una alerta.
  ALR_JOVE_ESPECIALITAT: (ctx, p) => (ctx.juvenils || [])
    .filter((j) => j.especialitat && j.sobra)
  ...
  // transferibles: no es tornen a llistar mai. Mentre estiguen a la plantilla cobren cada
  // setmana sense ocupar cap lloc del pla, i l'única eixida que els queda és despatxar-los.
  // Va agregada: són una decisió de plantilla, no onze avisos solts.
  ALR_DESPATXAR: (ctx, p) => {
    const seus = (ctx.jugadors || []).filter((j) => j.grup === 'despatxar');
    if (!seus.length) return [];
    const sou = seus.reduce((a, j) => a + (Number(j.sou) || 0), 0);
    return [alerta('ALR_DESPATXAR', null, 'alerta.despatxar', { n: seus.length, sou_total: sou },
      p.urgencia, null, ['sou_total'], 'n')];
  },

  // Compra accionable: buit d'entrenable amb el filtre concret i el pressupost màxim.
  ALR_COMPRA_ENTRENABLE: (ctx, p) => {
    // PLAÇA D'ENTRENAMENT BUIDA. Ix de les mateixes necessitats que pinta Mercat, no d'una
    // segona derivació: abans això comptava categories del PAS 6 mentres la pantalla mirava
    // l'assignació d'estructura, i les dues podien discrepar sobre què falta.
    //
    // I com allí: SENSE PREU DECLARAT no es diu «compra», es diu «mira quant costa». A
    // Hattrick el preu no el calcula el joc, i suggerir sense saber-lo és decidir a cegues.
    const buides = (ctx.compra?.necessitats || []).filter((n) => n.tipus === 'entrenable');
    return buides.map((n) => alerta('ALR_COMPRA_ENTRENABLE', null,
      n.preu == null ? 'alerta.compra_entrenable_preu' : 'alerta.compra_entrenable',
      { n: n.quants, nivell: n.nivell, preu: n.preu ?? 0 }, p.urgencia, null,
      n.preu == null ? null : ['preu'], 'n'));
  },
  ...
  // GASTES MÉS DEL QUE ENTRA. El sistema calculava el flux i no el deia enlloc: la píndola es
  // va llevar de la pantalla i el `sobrecost` —el mecanisme que hauria d'actuar— està tancat
  // mentre no hi haja prou setmanes declarades. El resultat era un dèficit real, calculat, i
  // en silenci.
  //
  // EL LLINDAR NO ÉS ZERO, ÉS LA RESERVA: el contracte diu que les despeses recurrents no
  // poden passar del (1 − `reserva_flux_pct`) dels ingressos. Menjar-se la reserva ja és el
  // senyal; el negatiu és el mateix senyal més fort, i per això és una regla amb dos textos.
  //
  // I NO ESPERA A CALIBRAR. Amb poques setmanes la mitjana és provisional i pot pujar, però la
  // despesa és exacta i el desequilibri és real HUI. Callar fins a la vuitena setmana seria
  // callar justament quan més falta fa dir-ho. La
  // provisionalitat es veu on viu el número: la targeta d'ingressos d'Economia ja diu sobre
  // quantes setmanes va la mitjana (invariant 18: vell ≠ absent ≠ zero).
  ALR_GASTA_MES: (ctx, p) => {
    const e = ctx.economia;
    if (!e || e.flux == null || e.ingressos_recurrents == null) return [];
    if (e.flux >= (e.reserva_flux ?? 0)) return [];
    const per = e.setmanes_periode || 1;
    const clau = e.flux < 0 ? 'alerta.gasta_mes' : 'alerta.gasta_reserva';
    return [alerta('ALR_GASTA_MES', null, clau, {
      ingressos: Math.round(e.ingressos_recurrents / per),
      despeses: Math.round(e.despeses_fixes / per),
      diferencia: Math.round(Math.abs(e.flux) / per),
    }, p.urgencia, null, ['ingressos', 'despeses', 'diferencia'])];
  },

  // ELS INGRESSOS JA SOSTENEN UN ENTRENADOR MILLOR. Ell és l'única palanca que multiplica el
  // motor de l'estratègia i era l'únic element que no escalava amb res: es restava sencer com
  // un fet, o siga que els ingressos podien duplicar-se i ningú diria mai que toca millorar-lo.
  //
  // NO ÉS UNA ACCIÓ IMMEDIATA: contractar-ne un es paga d'un colp i de CAIXA, com un fitxatge,
  // i el preu depén del lideratge que vulgues (taula de la guia). El que diu l'alerta és que ha
  // arribat el moment de decidir-ho, que abans no arribava mai.
  ALR_ENTRENADOR_MILLORABLE: (ctx, p) => {
    const e = ctx.economia?.entrenador;
    if (!e?.millorable) return [];
    return [alerta('ALR_ENTRENADOR_MILLORABLE', null, 'alerta.entrenador_millorable',
      { actual: e.nivell_actual, sostingut: e.nivell_sostingut, cost: Math.round(e.cost_sostingut) },
      p.urgencia, null, ['cost'])];
  },
