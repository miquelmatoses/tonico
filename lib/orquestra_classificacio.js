...
export async function classificaEquip(db, usuariId, equipId, plantilla) {
  ...
  // `sou_sostenible_setmanal`: `taula_salaris` va en €/setmana i el sostenible en unitats del
  // període. La conversió la fa l'economia, no cada consumidor (invariant 16).
  // La llista SENCERA (amb repeticions): vore la nota de pressupostSou.
  const nivells = nivellsObjectiu(llocs.map((l) => l.bucket), cfgPesos, eco.sou_sostenible_setmanal);
  // STOPPER (PAS 7): sense calibrar, el `sobrecost` no ordena res — és una xifra que penja del
  // flux, i el flux amb poques setmanes és soroll. Es torna 0 per a tots: la fila seguix
  // eixint, però cap jugador queda marcat com a sobrepagat per una lectura provisional.
  const puntsVenda = (j) => {
    if (!eco.calibrat) return 0;
