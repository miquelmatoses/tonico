...
export async function derivaLlistat(db, usuariId, dataInstantania, instId) {
  ...
  const { results: vendes } = await db.prepare(
    `SELECT v.jugador_id, v.estat FROM vendes v
  ...
  for (const f of files) {
    ...
    // `v.preu_venut == null` estava ací i sempre era cert: la columna es llegia i no
    // l'escrivia ningú des que l'import d'una venda no s'apunta (v3.1). Una condició que no
    // bifurca és una branca que ningú ha provat mai.
    } else if (v && v.estat === 'llistat') {
