...
export function classifica(jugadors, config, fixats = {}) {
  ...
  for (const cat of cats) {
    ...
    if (p.places) {                           // capacitat per bucket de posició
      ...
      // Ocupació de la posició per ALTRES categories (cobertura complementària). `resta_ocupacio_exclou`
      // llista categories que NO cobrixen (p.ex. entrenables: ocupen la seua posició natural però
      // entrenen en una altra, no fan de cos), així que no resten places de farciment.
      const exclou = new Set(p.resta_ocupacio_exclou || []);
      const ocupadaAltres = (bucket) => [...assignat].filter(([id, a]) => a.categoria !== cat.categoria && !exclou.has(a.categoria) && enPos(id, bucket)).length;
      ...
      // posició, se'n reté un encara que no complisca l'adequació (cobertura mínima dura).
