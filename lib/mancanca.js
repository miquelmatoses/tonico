...
//   excés(j)        = MAX(0; hab(j, habilitat_lloc) − nivell_objectiu_ht(lloc))
//
// EN L'ESCALA DE HATTRICK als dos costats. `nivell_objectiu` és l'índex de la taula de salaris
// (que comença en «Inadequate», el 5é esglaó) i `hab(j)` ve del CSV en l'escala sencera.
...
export function mancances(nivells, ocupants = {}) {
  ...
  for (const [lloc, n] of Object.entries(nivells)) {
    ...
    // ES COMPARA EN L'ESCALA DE HATTRICK, que és la del jugador. `nivell_objectiu` és l'índex
    // de la taula de salaris, que comença en «Inadequate» (el 5é esglaó): restar-lo d'un
    // `hab(jugador)` deixava TOTES les mancances quatre nivells curtes — un objectiu de
    // «Formidable» (HT 9) contra un jugador de CR 8 donava 0 en compte d'1.
    const objectiu = n.nivell_objectiu_ht ?? n.nivell_objectiu;
    const m = mancanca(objectiu, actual);
    out[lloc] = {
      ...n,
