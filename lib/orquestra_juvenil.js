...
import { entrenamentPrescrit } from './entrenament_places.js';
...
export async function configJuvenil(db, plantilla) {
  ...
  const pres = await entrenamentPrescrit(db, plantilla);
  return {
    ...
    velocitat: {
      [pres?.skill]: json(await cj(`velocitat_juvenil_${pres?.skill}`), null),
      [pres?.skill_b]: json(await cj(`velocitat_juvenil_${pres?.skill_b}`), null),
    },
    maxims: json(await cj('maxims_posicio'), {}),
    objectiu: Number(await pom('juvenil_objectiu')) || null,
    llistons: {
      [pres?.skill]: Number(await pom('entrenable_creativitat_min')),
      [pres?.skill_b]: Number(await pom('juvenil_passades_min')),
    },
    principal: pres?.skill ?? null,
    secundaria: pres?.skill_b ?? null,
  };
}
...
export async function plaJuvenilComplet(db, usuariId, files, plantilla) {
  ...
  const juvenils = files.map((f) => ({
    ...
    // BLOQUEJAT: lesionat o expulsat. No pot jugar, o siga que no entra a cap tall.
    bloquejat: esLesionat(f.lesio) || !!f.expulsat,
  }));
  ...
  const protegits = new Set(juvenils
    .filter((j) => avaluaHabilitat(j, cfg.principal, {
      llisto: cfg.llistons[cfg.principal], taula: cfg.velocitat[cfg.principal],
      subNivell: cfg.subNivell, anyDies: cfg.anyDies }).valor >= cfg.llistons[cfg.principal])
    .map((j) => j.jugador_id));
