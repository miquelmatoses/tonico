...
export function necessitats(est, opts = {}) {
  ...
  for (const l of est.onze) {
    ...
    if (-l.diferencia < distancia_min) continue;
    ...
    const p = perClau.get(clau) ?? { tipus: 'lloc', bucket: l.bucket, nivell: l.nivell_objectiu,
      clau, quants: 0, distancia: 0, motiu: 'distancia' };
    p.quants += 1;
    p.distancia = Math.max(p.distancia, -l.diferencia);   // el lloc que pitjor està del grup
    perClau.set(clau, p);
  }
