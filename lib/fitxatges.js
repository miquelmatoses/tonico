...
// I PORTA L'EDAT, perquè el preu d'un jugador de 32 no és el d'un de 26 amb el mateix perfil.
export const clauFitxatge = (tipus, bucket, detall, edat = null) =>
  (tipus === 'porter_suplent' ? 'porter_suplent'
    ...
    : tipus === 'entrenable' ? `entrenable:${detallClau(detall)}`
      : `${bucket}:${edat ?? 'x'}:${detallClau(detall)}`);
...
export function necessitats(est, opts = {}) {
  ...
  for (const l of distancia_min == null ? [] : est.onze) {
    ...
    const clau = clauFitxatge('lloc', l.bucket, l.perfil_objectiu, l.edat_objectiu) + (buit ? ':buit' : '');
    const p = perClau.get(clau) ?? { tipus: 'lloc', bucket: l.bucket, perfil: l.perfil_objectiu,
      edat: l.edat_objectiu ?? null, entrena: !!l.entrena,
...
export function cercaDe(n, opts = {}) {
  ...
  // el perfil s'ha calculat amb el descompte d'eixa edat: més jove costaria més del que tens,
  // més vell encara és més barat → mínim. Al que ENTRENA el que es compra és el recorregut:
  // més vell ja no el pots entrenar tant → màxim.
  return { ...base, perfil: n.perfil,
    ...(n.edat == null ? {} : n.entrena ? { edat_max: n.edat } : { edat_min: n.edat }) };
}
