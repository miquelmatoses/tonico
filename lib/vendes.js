...
// ES LLISTA UNA VEGADA. Si va desert, estes són les eixides del full; per a un SOBRANT la
// recomanada és despatxar-lo, perquè rellistar-lo cada setmana és pagar `cost_llistat` per res.
export const EIXIDES_DESERTA = ['rebaixar', 'rellistar', 'despatxar', 'un_euro'];

// El dispar és un fet de les INSTANTÀNIES (transferible 1 → buit), no de la fitxa. Lligar-lo
// a l'estat de la fitxa no funcionava mai: la fitxa es queda en 'llistat' precisament fins
// que algú responga, així que la condició «ja no està llistat» era sempre falsa.
export function preguntaVenda({ transferible_abans, transferible_ara, venda_apuntada = false,
  es_sobrant = false } = {}) {
  const eraLlistat = Number(transferible_abans) === 1;
  const jaNoHoEs = Number(transferible_ara) !== 1;       // buit, 0 o absent
  if (!eraLlistat || !jaNoHoEs) return null;
  if (venda_apuntada) return null;                        // ja sabem què va passar
  return { pregunta: 'venut_o_deserta', eixides: EIXIDES_DESERTA,
    recomanada: es_sobrant ? 'despatxar' : null };
}

// destí(deserta): un SOBRANT que ha anat desert s'acomiada — no té lloc a cap dels dos onzes
// i cada rellistat torna a cobrar la taxa. Un retingut, un rotatiu o un cos MAI: per a ells la
// subhasta deserta no és un veredicte, és que el preu no era el bo.
export function destiDeserta({ es_sobrant = false } = {}) {
  return es_sobrant ? { accio: 'despatxa', motiu: 'sobrant_desert' }
    : { accio: 'pregunta', motiu: 'retingut', eixides: EIXIDES_DESERTA };
}
