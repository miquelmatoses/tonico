...
export async function marcaDeserts(db, usuariId) {
  ...
  const { results: insts } = await db.prepare(
    'SELECT id FROM instantanies WHERE equip_id=? ORDER BY data DESC, id DESC LIMIT 2'
  ).bind(equip.id).all();
  if (insts.length === 2) {
    const { results: files } = await db.prepare(
      `SELECT ara.jugador_id, ara.transferible AS ara, abans.transferible AS abans
         FROM instantanies_jugadors ara
         JOIN instantanies_jugadors abans ON abans.jugador_id = ara.jugador_id AND abans.instantania_id = ?
        WHERE ara.instantania_id = ?`
    ).bind(insts[1].id, insts[0].id).all();
    // `en_plantilla` és cert per construcció: el JOIN només casa els que seguixen a la
    // instantània d'ara. Els que han desaparegut els ha comprat algú i tenen un altre camí.
    for (const f of files) {
      if (!subhastaDeserta({ transferible_abans: f.abans, transferible_ara: f.ara })) continue;
      await db.prepare(
